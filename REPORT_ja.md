...
- **課題**: QWERTYキーボードのアルファベット25キー（q → w → e → … → n）を順番に**3回打鍵**し、各キーごとに約11秒間計測
...
各記録に3回の打鍵が含まれており、波形中に3つのバーストが確認できるキーが多い。上段のキー（q, w, e, r, t, y付近）で活動量が高い傾向が視覚的に確認できる。なお積分値は3回分の合計であるため、1打鍵あたりの活動量は表示値の約1/3となる。
...
| 課題 | 詳細 |
|------|------|
| サンプル数 | 被験者1名・各キー3打鍵1セットのみ。複数セット・複数被験者での検証が必要 |
